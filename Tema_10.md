...
```
### Результат
![Меню](https://github.com/asrortxt/Software_Engineering/blob/Тема_10/pic/Lab10_1.PNG)

## Выводы

1. Без декоратора @lru_cache - вычисление fibonacci(100) выполняется крайне медленно из-за экспоненциального роста повторяющихся операций.
2. С декоратором @lru_cache - результат получается мгновенно благодаря сохранению промежуточных результатов.
3. Оптимизация производительности - использование кэша снижает вычислительную сложность с экспоненциальной до линейной.
...
```
### Результат
![Меню](https://github.com/asrortxt/Software_Engineering/blob/Тема_10/pic/Lab10_2.PNG)

## Выводы

1. Селективная обработка - декоратор анализирует только первые два аргумента, остальные пропускает без изменений
2. Проверка возраста - валидирует корректность возрастного диапазона (0 < возраст < 130).
3. Корректировка данных - автоматически заменяет некорректные значения возраста на предупреждающее сообщение.
4. Универсальность - совместим с функциями, принимающими разное количество параметров
...
```
### Результат
![Меню](https://github.com/asrortxt/Software_Engineering/blob/Тема_10/pic/Lab10_3.PNG)
...
```
### Результат
![Меню](https://github.com/asrortxt/Software_Engineering/blob/Тема_10/pic/Lab10_4.PNG)
...
```
### Результат
![Меню](https://github.com/asrortxt/Software_Engineering/blob/Тема_10/pic/Lab10_5.PNG)
...
```
### Результат
![Меню](https://github.com/asrortxt/Software_Engineering/blob/Тема_10/pic/Solo10_1.PNG)
...
```
### Результат
![Меню](https://github.com/asrortxt/Software_Engineering/blob/Тема_10/pic/Solo10_2.PNG)
...
```
### Результат
![Меню](https://github.com/asrortxt/Software_Engineering/blob/Тема_10/pic/Solo10_3.PNG)
...
```
### Результат
![Меню](https://github.com/asrortxt/Software_Engineering/blob/Тема_10/pic/Solo10_4.PNG)

## Выводы

1. Один декоратор работает с разными функциями.
2. Накопление состояния - декоратор запоминает общую статистику.
3. Мониторинг производительности - измеряет время выполнения каждой функции
...
```
### Результат
![Меню](https://github.com/asrortxt/Software_Engineering/blob/Тема_10/pic/Solo10_5.PNG)

## Выводы

1. Единая обработка ошибок - одно исключение для разных типов невалидных данных.
2. Четкая спецификация - конкретные сообщения об ошибках для разных случаев.
